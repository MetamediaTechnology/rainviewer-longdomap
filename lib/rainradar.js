function RainRadar(map, options) {
  ...
  function changeLayer(action) {
    ...
    const _url = URL
    const longdoTile = document.getElementsByClassName('ldmap_maparea');
    for (let i = 0; i < longdoTile.length; i++) {
      const tileUrl = longdoTile[i].src
      if (tileUrl) {
        const {
          hostname
        } = new _url(tileUrl)
